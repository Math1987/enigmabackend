...
const TABLE_CALCULATION: string = "calcul";

export class Calculation {
  static initCalculation(callBack) {
    Data.CONNECTION.query(
      `
          create table if not exists ${TABLE_CALCULATION}
          (
          name VARCHAR(154), 
  ...
  static readCalculs(callBack) {
    console.log("READ CALCULS");
    Data.successOrFail(
      `
      SELECT * FROM ${TABLE_CALCULATION}
    `,
      (res) => {
        if (res) {
          let obj = {};
  ...
  static createCalculations(calculations, callBack) {
    Data.CONNECTION.query(
      `
  INSERT IGNORE INTO ${TABLE_CALCULATION}
  ...
  static readAllCalculations(callBack) {
    Data.CONNECTION.query(
      `
  SELECT * FROM ${TABLE_CALCULATION}
  `,
  ...
  static readCalculationsByName(name, callBack) {
    Data.CONNECTION.query(
      `
  SELECT * FROM ${TABLE_CALCULATION}
  WHERE name = "${name}"
  ...
  static updateCalculation(name, attribute, value, callBack) {
    Data.CONNECTION.query(
      `
  UPDATE ${TABLE_CALCULATION}
  SET value = ${value}
