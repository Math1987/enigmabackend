...
class ValuesData {
    static buildTable(datas, callBack) {
        ...
        CREATE TABLE IF NOT EXISTS ${datas.name}_${values_data_1.ValuesData.TABLE_VALUES}
        ( 
        id VARCHAR(36),
        key_ VARCHAR(36),
        value FLOAT,
    ...
    static readResources(id, world_name, callBack) {
        data_1.Data.successOrFail(`
        SELECT * FROM ${world_name}_${values_data_1.ValuesData.TABLE_VALUES}
        WHERE id = "${id}"
        `, function (res) {
            if (res && res.length > 0) {
    ...
    static readAsObject(world_name, id, callBack) {
        values_data_1.ValuesData.readResources(id, world_name, (res) => {
            if (res && res.length > 0) {
                let resAsObject = {};
                for (let row of res) {
                    resAsObject[row.key_] = row.value;
                }
                callBack(resAsObject);
            }
            else {
                callBack(null);
            }
    ...
    static addValue(id, world_name, key_, adder) {
        return new Promise((resolve, reject) => {
           ...
           UPDATE ${world_name}_${values_data_1.ValuesData.TABLE_VALUES}
    ...
    static updateValues(id, world_name, keysAdds) {
        ...
        return new Promise((resolve, reject) => {
            data_1.Data.successOrFail(`
               INSERT INTO ${world_name}_${values_data_1.ValuesData.TABLE_VALUES} (id, key_, value)
               VALUES ${vals}
