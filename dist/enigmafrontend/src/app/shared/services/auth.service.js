...
let AuthService = AuthService_1 = class AuthService {
     ...
     */
    initTimer() {
        return rxjs_1.timer(30000, 15000).pipe(operators_1.switchMap(() => {
            if (localStorage.getItem(AuthService_1.LOCAL_JWT)) {
                return this.http.get(`${environment_1.environment.apiURL}/refreshToken`).pipe(operators_1.tap((token) => {
                    this.jwtToken.next({
                        isAuthenticated: true,
                        token
                    });
    ...
    newToken(values) {
        return this.http.post(`${environment_1.environment.apiURL}/newToken`, values).pipe(operators_1.tap((token) => {
            if (token) {
                this.jwtToken.next({
                    isAuthenticated: true,
                    token: token
     ...
     */
    signIn(credentials) {
        return this.http.post(`${environment_1.environment.apiURL}/signin`, credentials).pipe(operators_1.tap((token) => {
            localStorage.setItem(AuthService_1.LOCAL_JWT, token);
            this.jwtToken.next({
                isAuthenticated: true,
                token
